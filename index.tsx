...
import { 
  ...
  ShieldCheck, RefreshCw, AlertCircle, Github, GitBranch, ExternalLink
} from 'lucide-react';
...
const Button = ({ children, onClick, variant = 'primary', className = '', disabled = false }: any) => {
  ...
  const variants = {
    ...
    github: "bg-[#24292e] hover:bg-[#2f363d] text-white shadow-lg"
  };
  return (
    <button 
      onClick={onClick} 
...
// --- GitHub Manager Component ---
const GitHubManager = ({ site, user }: { site: SiteData, user: User }) => {
  const [token, setToken] = useState(() => localStorage.getItem('github_pat') || '');
  const [repoName, setRepoName] = useState(site.slug || 'my-landing-page');
  ...
  const handlePush = async () => {
    ...
    try {
      ...
      setStatusMsg('Checking repository...');
      let repo;
      try {
        ...
        const { data } = await octokit.rest.repos.createForAuthenticatedUser({ 
          name: repoName,
          description: `Landing page for ${site.title} created with LaunchAI`,
...
<head>
  ...
  ${site.content.tailwind ? '<script src="https://cdn.tailwindcss.com"></script>' : ''}
  <style>
    /* Base Resets */
    body { margin: 0; padding: 0; font-family: system-ui, -apple-system, sans-serif; }
    ${site.content.css || ''}
...
<body>
  ${site.content.html}
  <script>${site.content.scripts || ''}</script>
  <!-- Generated by LaunchAI -->
</body>
</html>`;

      // Helper to handle unicode string to base64
      const toBase64 = (str: string) => window.btoa(unescape(encodeURIComponent(str)));

      // 4. Update index.html
      setStatusMsg('Pushing code...');
      
      // Get SHA if exists
      let sha;
      try {
        const { data } = await octokit.rest.repos.getContent({ owner, repo: repoName, path: 'index.html' });
        sha = (data as any).sha;
      } catch (e) {
      ...
      await octokit.rest.repos.createOrUpdateFileContents({
        ...
        repo: repoName,
        path: 'index.html',
        message: `Deploy ${site.title} via LaunchAI`,
        content: toBase64(htmlContent),
        sha,
        committer: { name: 'LaunchAI', email: 'deploy@launchai.com' }
      });

      setStatus('success');
      setStatusMsg('Successfully pushed to GitHub!');
      setRepoUrl(repo.html_url);
    } catch (e: any) {
      console.error(e);
      setStatus('error');
      setStatusMsg(e.message || 'Failed to push to GitHub');
      if (e.status === 401) {
        setStatusMsg('Invalid Token. Please check your PAT.');
      ...
      {showTokenInput ? (
        <div className="space-y-3 bg-gray-50 p-3 rounded-lg border border-gray-200">
           <p className="text-xs text-gray-600">
             To push code, you need a <strong>Personal Access Token</strong> (Classic) with <code>repo</code> scope.
           </p>
           <input 
             type="password" 
           ...
           <div className="flex gap-2">
             <Button onClick={saveToken} disabled={!token} className="flex-1 py-1 text-xs h-8">
               Save Token
             </Button>
             <a 
               href="https://github.com/settings/tokens/new?scopes=repo&description=LaunchAI+Deploy" 
               target="_blank" 
               rel="noreferrer"
               className="flex items-center gap-1 px-3 py-1 bg-white border border-gray-300 rounded text-xs text-gray-700 hover:bg-gray-50"
             >
        ...
        <div className="space-y-3">
           <div className="flex items-center gap-2">
             <div className="flex-1">
               <label className="block text-xs font-medium text-gray-500 mb-1">Repository Name</label>
               <input 
                type="text" 
                className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 outline-none font-mono"
                value={repoName}
                onChange={(e) => setRepoName(e.target.value)}
                placeholder="my-awesome-site"
               />
             </div>
             <button onClick={() => setShowTokenInput(true)} className="text-xs text-blue-600 underline self-end mb-2">
               Change Token
             </button>
           </div>
           ...
           >
             {isPushing ? <><Loader2 className="animate-spin w-4 h-4"/> {statusMsg}</> : <><GitBranch size={16} /> Push to GitHub</>}
           </Button>

           {status === 'success' && (
             <div className="p-2 bg-green-50 border border-green-200 rounded text-xs text-green-800 flex flex-col gap-1">
               <div className="flex items-center gap-1 font-bold"><CheckCircle size={12}/> Deployed Successfully</div>
               <a href={repoUrl} target="_blank" rel="noreferrer" className="underline truncate block">{repoUrl}</a>
             </div>
           )}
           
           {status === 'error' && (
             <div className="p-2 bg-red-50 border border-red-200 rounded text-xs text-red-600 flex items-start gap-1">
...
const Dashboard = ({ user, userRole }: { user: User, userRole: string }) => {
  ...
  const downloadSite = () => {
    if (!currentSite) return;
    const content = currentSite.content;
    const htmlContent = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  ...
  <!-- Generated by LaunchAI -->
    ...
    a.href = url;
    a.download = `${content.slug || 'landing-page'}.html`;
                ...
                {!currentSite ? (
                  ...
                  <div className="space-y-6 animate-fade-in">
                     ...
                     <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-3">
                         <h3 className="font-bold text-gray-900 text-sm">Deployment Options</h3>
                         ...
                         {currentSite.isPublished ? (
                            <div className="space-y-3">
                                ...
                                <GitHubManager site={currentSite} user={user} />
                            </div>
                         ) : (
                             <Button onClick={publishSite} className="w-full bg-green-600 hover:bg-green-700 text-white shadow-lg text-sm py-2">
                               <Rocket size={16} /> Enable Public Link
                             </Button>
                         )}

                         <div className="border-t border-gray-200 pt-3">
                            <p className="text-xs text-gray-500 mb-2">Want to host this yourself?</p>
                            <Button onClick={downloadSite} variant="secondary" className="w-full text-sm py-2">
                               <Download size={16} /> Download HTML Code
                            </Button>
                            <p className="text-[10px] text-gray-400 mt-1 text-center">
                               Download index.html to deploy on Netlify, Vercel, or any hosting.
                            </p>
                         </div>
                     </div>
                     
                     <div className="pt-2">
                        <Button onClick={() => { setCurrentSite(null); setPrompt(''); }} variant="ghost" className="w-full text-gray-500 hover:text-red-600 hover:bg-red-50 text-sm">
                            Delete & Start Over
                        </Button>
                     </div>
                  </div>
